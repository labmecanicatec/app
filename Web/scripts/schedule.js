...
function Schedule(opts, resourceGroups) {
  ...
  function renderEvents(clear = false) {
    ...
    function findStartAndEnd(res, table, startAttribute, endAttribute) {
      let startTd = table.find(
        'td[data-resourceid="' + res.ResourceId + '"][data-min="' + res[startAttribute] + '"]:first'
      );
      let endTd = table.find(
                  ...
                  for (const reservation of resourceIdMap.get(resourceId)) {
                    if (isReservationInTable(reservation, t)) {
                      ...
                      const isDraggable =
                        ...
                        ((reservation.IsOwner && !reservation.IsPast) || reservation.IsAdmin);
                      if (isDraggable) {
                        div.on(
                          'dragstart',
          ...
          $('#reservations')
            ...
            .each(function () {
              ...
              if (getNumberOfDaysInReservation(res.StartDate, res.EndDate) == 1) {
                current_TD = t.find(
                  'td[data-resourceid="' + res.ResourceId + '"][data-min="' + res['StartDate'] + '"]:first'
                );
              } else {
        ...
        $('#reservations')
          ...
          .each(function () {
            ...
            if (opts.scheduleStyle === ScheduleStandard) {
              ...
              if (trAdjusted[currentTrId] === false) {
                //no sense in setting the row height multiple times because it will always be the same so do a check and set the height once per row with reservations
                if (current_TR.height() <= trHeights[currentTrId]) {
                  if (
                    scheduleOpts.resourceMaxConcurrentReservations[res.ResourceId] > 1 &&
                    className != 'unreservable'
                  ) {
                    //takes into account possible existence of concurrent reservations
            ...
            if (opts.scheduleStyle === ScheduleTall) {
              ...
              if (numberOfConflicts > 0) {
                ...
                if (opts.scheduleStyle === ScheduleStandard) {
                  if (
                    scheduleOpts.resourceMaxConcurrentReservations[res.ResourceId] !== numberOfConflicts + 1 &&
                    scheduleOpts.resourceMaxConcurrentReservations[res.ResourceId] > 2
                  ) {
    ...
    $('#print_schedule').on('click', function (e) {
      ...
      html2canvas(element).then(function (canvas) {
        ...
        $(newWindow.document.body)
          .html("<img id='print-schedule-image' src=" + tmpImage + " style='width:100%;'></img>")
          .ready(function () {
            newWindow.focus();
            newWindow.print();
            newWindow.close();
  ...
  function initReservable() {
    ...
    if (options.disableSelectable != '1') {
      ...
      function removeIfNonSequential(td) {
        tds.forEach((i) => {
          if (
            Number.parseInt(i.data('min')) > Number.parseInt(td.data('min')) ||
      ...
      reservations.on('mouseup', 'td.reservable', (e) => {
        if (selectingTds) {
          e.stopPropagation();
          if (
            Number.parseInt(firstTd.data('min')) < Number.parseInt(lastTd.data('min')) &&
