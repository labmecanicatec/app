function ReservationManagement(opts, approval) {
  ...
  ReservationManagement.prototype.init = function () {
    ...
    elements.reservationTable.on('click', 'tr.editable', function (e) {
      if (
        ...
        $(e.target).closest('td').hasClass('action')
      ) {
        e.stopPropagation();
        return;
      }
    ...
    ConfigureAsyncForm(elements.deleteInstanceForm, getDeleteUrl, null, deleteReservationResponseHandler, {
      dataType: 'json',
    });
    ConfigureAsyncForm(elements.deleteSeriesForm, getDeleteUrl, null, deleteReservationResponseHandler, {
      dataType: 'json',
    });
    ...
    ConfigureAsyncForm(
      elements.importReservationsForm,
      defaultSubmitCallback(elements.importReservationsForm),
      importHandler
    );
  ...
  function filterReservations() {
    ...
    var filterQuery =
      ...
      elements.userId.val() +
      '&un=' +
      elements.userFilter.val() +
      '&rn=' +
      elements.referenceNumber.val() +
      '&rsid=' +
      elements.statusId.val() +
      '&rrsid=' +
      elements.resourceStatusIdFilter.val() +
      '&rrsrid=' +
      reasonId +
      '&rtitle=' +
      elements.title.val() +
      ...
      '&in=' +
      toInt(elements.missedCheckin.is(':checked')) +
      '&out=' +
      toInt(elements.missedCheckout.is(':checked'));
